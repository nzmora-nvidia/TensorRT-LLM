...
@torch.library.custom_op("auto_deploy::torch_attention_prepare_fused_mla_metadata", mutates_args=())
def torch_attention_prepare_fused_mla_metadata(
    ...
    return (
        ...
        seq_start,
        position_ids[:num_seq].clone(),
    )


# Taken directly from test_deepseek_attn.py
@torch_attention_prepare_fused_mla_metadata.register_fake
def torch_attention_prepare_fused_mla_metadata_fake(
    ...
    return (
        ...
        torch.empty_like(seq_len[:num_seq]),
        torch.empty_like(position_ids[:num_seq]),
    )


@AttentionRegistry.register("TorchMLABackend")
class TorchMLABackend(AttentionDescriptor):
    @classmethod
    ...
    def get_num_qkv_args(cls) -> int:
        ...
        return 10  # (q_normed_dn, compressed_kv, k_pe), (sin_cache, cos_cache), (wkv_b, wq_b, w_uq_ukv, wo_proj, w_uv_o)
    ...
    def get_cached_attention_op(cls) -> MHACallable:
        ...
        return torch.ops.auto_deploy.torch_deepseek_mla_with_kv_cache

    @classmethod
    def get_prepare_metadata_op(cls) -> Tuple[PrepareMetadataCallable, int]:
        return torch.ops.auto_deploy.torch_attention_prepare_fused_mla_metadata, 5
    ...
    def get_cache_initializers(
        ...
        def _create_ckv_cache(si: SequenceInfo):
            ...
            #     si.num_pages, si.page_size, head_dim_ckv, si.device, dtype=cache_config.dtype or kv_fake.dtype, init_randn=False
            # )
            return _create_linear_caches(
                si.max_batch_size,
                si.max_seq_len,
                head_dim_ckv,
                si.device,
        ...
        def _create_kpe_cache(si: SequenceInfo):
            head_dim_kpe = 64
            # return _create_paged_cache(
            #     si.num_pages, si.page_size, head_dim_kpe, si.device, dtype=cache_config.dtype or kv_fake.dtype, init_randn=False
            # )
            return _create_linear_caches(
                si.max_batch_size,
                si.max_seq_len,
    ...
    def get_constants(cls, source_attn_node: Node) -> List[Constant]:
        # attention_mask = source_attn_node.kwargs.get("attention_mask", None)
        # softmax_scale = source_attn_node.kwargs.get("softmax_scale", None)
        softmax_scale = source_attn_node.args[-1]
        # attention_mask = source_attn_node.args[-2]
